...
class KNNBasic(SymmetricAlgo):
    ...
    def __init__(self, dim, k=40, min_k=1, sim_options={}, verbose=True, user_based=True, **kwargs):
        ...
        self.min_k = min_k
        self.dim = dim
        self.similarity = defaultdict(list)
        if user_based:
            self.means = np.zeros(self.dim, dtype=np.float16)

    def fit(self, trainset, batch_start):

        SymmetricAlgo.fit(self, trainset)
        sim = self.compute_similarities()

        print('assembling similarity matrix for batch_start from %d\n' % (batch_start))
        for i in range(self.n_x):
            for j in range(i, self.n_x):
                if math.fabs(sim[i, j]) > 0.00001:
                    self.similarity[i + batch_start].append((j + batch_start, sim[i, j]))
                    self.similarity[j + batch_start].append((i + batch_start, sim[i, j]))
        print('finish assembling similarity matrix for batch_start from %d\n' % (batch_start))
    ...
    def estimate(self, u, i):
        ...
        for (x2, r) in self.yr[y]:
            for i in range(len(self.similarity[x2])):
                if x == self.similarity[x2][i][0]:
                    neighbors.append((x2, self.similarity[x2][i][1], r))
...
class KNNWithMeans(SymmetricAlgo):
    ...
    def fit(self, trainset, batch_start):
        ...
        for x, ratings in iteritems(self.xr):
            self.means[x+batch_start] = np.mean([r for (_, r) in ratings])
    ...
    def estimate(self, u, i):
        ...
        # neighbors = [(x2, self.similarity[x, x2], r) for (x2, r) in self.yr[y]]
